...
import { useMyPerformanceGoals, useCreatePerformanceGoal, useUpdateGoalProgress, useDeletePerformanceGoal } from '@/hooks/performance/usePerformanceGoalsQuery';

interface GoalsSectionProps {}

const GoalsSection = ({}: GoalsSectionProps) => {
  const { data: goals, isLoading } = useMyPerformanceGoals();
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <Button onClick={handleCreateGoal} className="w-full" disabled={createGoal.isPending}>
                {createGoal.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Plus className="w-4 h-4 mr-2" />}
                Create Goal
              </Button>
            </div>
          </div>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-1">
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => updateProgress.mutate({ id: goal.id, progress: Math.min(100, goal.progress + 10) })}
